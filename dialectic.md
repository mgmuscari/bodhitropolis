...
This template repository codifies **Dialectic** — a structured, dialectical, AI-assisted software development lifecycle designed for solo developers and small teams building with Claude Code, OpenCode, Cursor, Windsurf, Cline, Aider, or other agentic coding tools.

The name points at an older idea. Socratic elenchus and Hegelian thesis-antithesis-synthesis both treat truth as something that emerges from structured disagreement rather than from solo reasoning. A proposer advances a position; an interlocutor tests it by probing for contradictions and unexamined assumptions; what survives is more defensible than either agent's first draft. This methodology applies that pattern to software: agent pairs probe each other's work at every stage of development, and the code that reaches `main` is the synthesis that survived contact.

The core thesis: **structured tension between interlocutors produces better outcomes than single-agent workflows at every stage of the development lifecycle.** Features are isolated into PRDs (Product Requirements Documents), implementation is isolated into PRPs (Product Requirements Prompts), and dialectic review gates separate each stage — ensuring that no code reaches `main` without surviving cross-examination from an interlocutor whose job is to sense where it yields.
...
This is not a prompt library, a CLAUDE.md tutorial, or a collection of slash commands. It's a complete development *practice* — in the methodological sense — that happens to be implemented as a GitHub template repository. The repository structure, the documentation conventions, the git workflow, and the dialectic review patterns are all load-bearing.
...
The missing piece is **dialectic** — a second agent (or a second invocation with a different stance) whose job is to sense where the structure yields. Not to attack, but to test. This is how Anthropic's own engineering team uses Claude Code internally. It's how security-focused teams run audits. It's a pattern that works across domains because it exploits the same mechanism: calibrated pressure between interlocutors reveals weaknesses that solo practice never finds.
...
- **A dueling-agent security audit process** — Dueling agents (auditor vs. skeptical client) producing tighter, more defensible reports.
...
1. **Tension is a practice, not a problem.** Every stage has a proposer and an interlocutor. They test each other by design. The output is neither agent's work — it's the synthesis that survives contact.
...
6. **The name is load-bearing.** Naming the practice "dialectic" isn't branding — it's a functional context prime. Models operating under a practice with lineage and discipline produce different (better) output than models executing a task list.
...
│       ├── review-plan.md             # Dialectic plan review
│       ├── execute-prp.md             # PRP execution with validation
│       ├── review-code.md             # Dialectic code review
...
│   ├── reviews/                       # Dialectic review artifacts
...
│       └── dialectic-review.yml      # Triggered on PR, runs interlocutor
│
└── .gitignore
```
...
The development lifecycle has four tiers. Choose the tier that matches the size and sensitivity of the change:

```
Light Tier (bug fixes, config tweaks, docs):
    Feature Branch → Implement → /review-code (optional) → PR → Merge

Iterative Tier (performance, accuracy, or debug loops):
    Feature Brief → /investigate → Implement (TDD)
        → Benchmark/Test → Fix → Loop
            → /review-code → PR → Merge
...
Each arrow is a gate. Work does not proceed until the gate passes. Gates are not rubber stamps — the interlocutor can reject, request changes, or flag structural weaknesses that send work back to a previous stage.

Tier metadata is stored in a `.dialectic-tier` file on the feature branch (containing `light`, `iterative`, `standard`, or `full`). This file must never reach `main` — `close-feature.sh` removes it before the squash-merge.
...
| **Iterative** | Work driven by empirical feedback loops. Developer is present, directing rapid edit-test-fix cycles. | "Optimize inference latency", "Improve BFCL accuracy", "Debug grammar compilation failures" |
...
**Iterative tier** is designed for benchmark-driven optimization, performance investigation, and bug hunt loops. Unlike light tier, TDD is mandatory and code review is required before PR. Unlike standard tier, no PRD/PRP or team mode is required — the developer is the interlocutor in real-time. Use `/investigate` to enter this workflow.
...
- **Light → Standard:** Edit `.dialectic-tier` to `standard` and commit. Run `/new-feature <description>` — it will detect the existing branch and generate the PRD. Continue with `/generate-prp`, etc.
- **Light → Full:** Same as light→standard, then continue through the full pipeline including `/security-audit`.
- **Standard → Full:** Edit `.dialectic-tier` to `full` and commit. All standard artifacts already exist. Run `/security-audit` before closing the feature.
- **Downgrading (e.g., standard → light):** Not recommended — artifacts already created aren't removed. But you can edit `.dialectic-tier` to `light` and `close-feature.sh` will skip artifact checks accordingly.
...
|-------------|----------|--------|--------------|
| `/review-plan-team <prp>` | `/review-plan` | Proposer + Interlocutor | Simultaneous start, message exchange |
...
| `/review-code-team` | `/review-code` | Code Reviewer + Implementation Defender | Finding-by-finding exchange, 2-round max |
...
Agent definitions in `.claude/agents/` enforce stance constraints at the tool level — the interlocutor literally cannot write files, not just prompt-instructed not to. See `docs/guides/agent-teams.md` for setup, cost implications, known bugs, workarounds, and usage guidance.

**Known team bugs and hardening:**
- Agent definitions load for team members (bug #24316 resolved, verified 2026-04-22) — the inline-stance workaround has been removed; keep body-referenced tools in each stance's `tools:` grant
- Model inheritance is unreliable (bug #32368) — all team spawns require explicit `model: "opus"` (re-evaluated 2026-06-01: guard retained, not verified fixed)
...
The Dialectic methodology uses a three-layer architecture to support multiple AI coding platforms:
...
#### 4.1.5 Scaling the Dialectic with Dynamic Workflows

The four tiers and team mode all operate on a **single change** — one feature, one PR diff. Some
work is larger than any single context window: a security sweep of an entire service, a bug hunt
across hundreds of thousands of lines, an API deprecation spanning hundreds of call sites. For this,
the Dialectic adds a **scale tier** built on Claude Code's dynamic workflows (the `Workflow` tool).

A dynamic workflow is the same dialectic, fanned out and made resumable. Where team mode stages a
*live* proposer↔interlocutor exchange the team lead participates in, a workflow scripts the exchange
across the whole codebase: **finders propose** (thesis), **independent skeptics try to refute each
finding** (antithesis), and **only what survives cross-examination is reported** (synthesis). The
canonical workflow shape — find → adversarially verify → synthesize — *is* elenchus at scale.

Two commands ship:

| Command | Purpose | Shape |
|---------|---------|-------|
| `/dialectic-sweep` | Codebase-scale audit (bug/security/quality) | One finder per dimension → N skeptics refute each finding → ranked report |
| `/dialectic-migrate` | Codebase-scale mechanical change | Discover sites → transform each in worktree isolation → reviewer cross-examines each patch |

Properties that distinguish the scale tier from team mode:

- **Background and resumable.** A run that is interrupted resumes from its journal rather than
  restarting. Progress is durable.
- **Not human-in-the-loop during the run.** The exchange is scripted, not conversational — the
  trade for reaching codebase scale. Results return as one consolidated artifact (`docs/audits/`).
- **Its own tier.** Migrations edit code directly; set `.dialectic-tier` to `workflow` (on the
  `block-solo-implementation.sh` allow-list) so the methodology guard permits it.
- **Token-intensive.** A sweep spawns one finder per dimension plus several skeptics per finding.
  Scope tightly first, then widen.
- **Claude Code only.** The `Workflow` tool has no OpenCode/portable equivalent; on those backends,
  scope work down to a branch and use team/sequential. This is a deliberate divergence — Claude
  Code is the reference implementation.

Teams remain the primary substrate for per-change work; the scale tier *adds* capability for work
that no single PR can hold. See `docs/guides/dynamic-workflows.md`.
...
### 4.4 Stage 3: Dialectic Plan Review

**Command:** `/review-plan docs/PRPs/<slug>.md`

**What happens:**

1. A **second agent invocation** reads the PRP with a different stance — the "Interlocutor" role (defined in AGENTS.md)
2. The interlocutor's job is to sense where the structure yields:
   - Missing edge cases
...
**Interlocutor Stance (from AGENTS.md):**

The interlocutor is explicitly instructed to probe for structural weakness, to assume the plan has at least three yield points, and to check every file path, symbol reference, and API assumption against the actual codebase. The interlocutor does not write code — it only tests the plan's balance.

**Why this works:** The proposer optimizes for completeness and coherence. The interlocutor optimizes for correctness and resilience. These are genuinely different optimization targets, and the tension between them reveals problems that either stance alone would miss. Like in Socratic dialectic — you can't find your own blind spots. You need contact with a partner who's listening for them.
...
### 4.6 Stage 5: Dialectic Code Review
...
This is where dialectic shifts to a harder frame. The security audit deliberately drops the cooperative interlocutor dynamic and adopts an adversarial stance — because security testing requires a model that's trying to break things, not just sense weakness.
...
Note: This is a deliberate shift from dialectic to a harder adversarial frame.
...
**`scripts/new-feature.sh <name> [--tier light|iterative|standard|full]`**
1. `git checkout main && git pull`
2. `git checkout -b feature/<name>`
3. Writes tier to `.dialectic-tier` (default: `standard`)
4. For standard/full: creates `docs/PRDs/<name>.md` from template; for light/iterative: skips PRD
5. Initial commit: `docs: scaffold PRD for <name>` (standard/full) or `chore: create feature branch for <name> (<tier> tier)` (light/iterative)

**`scripts/close-feature.sh <name>`**
1. Reads `.dialectic-tier` to determine tier
2. Verifies review artifacts exist (tier-appropriate: light/iterative skip PRD/PRP, standard checks PRD/PRP/reviews, full additionally checks audit)
3. Verifies all tests pass
4. Squash-merges feature branch to main, removing `.dialectic-tier` from staging before commit
5. Cleans up branch
...
### Dialectic as Agent Orchestration
...
Adding a second agent with a different stance changes the optimization landscape. The proposer now knows its work will be tested, which (via the training dynamics of instruction-tuned models) produces more careful initial work. The interlocutor, freed from the need to be constructive or comprehensive, can focus purely on sensing where the structure gives.

This is the same dynamic that makes:

- **Socratic elenchus and Hegelian dialectic** effective in philosophy and law — the position that survives structured questioning is the one you can trust
...
The critical insight: **the name matters.** "Adversarial review" primes a model toward attack and fault-finding. "Dialectic review" primes toward sensing and structural testing. The second frame produces output with different attentional qualities — more patient, more attuned to subtle imbalances, less focused on surface-level catches. The stance definitions in AGENTS.md aren't just system prompts; they're context primes that shape the character of the model's attention. This is a known property of LLM behavior (role priming / narrative framing) applied deliberately as an engineering tool.
...
For teams adopting this template:

- **First-pass implementation rate:** % of PRPs that result in working code without human intervention. Target: 80%+.
- **Defect escape rate:** % of bugs found in production that were NOT caught by dialectic review. Target: <10%.
...
- GitHub Actions workflow for automated dialectic review on PR
...
- Agent team mode for concurrent proposer/interlocutor workflows
...
| Plan Review | `/review-plan <prp>` | Standard, Full | PRP path | `docs/reviews/plans/<slug>.md` | Interlocutor |
...
| Concurrent Plan Review | `/review-plan-team <prp>` | Standard, Full | PRP path | Revised PRP + `docs/reviews/plans/<slug>.md` | Team: Proposer + Interlocutor |
...
| Concurrent Code Review | `/review-code-team` | Standard, Full | Branch diff + PRP | Revised code + `docs/reviews/code/<slug>.md` | Team: Code Reviewer + Defender |
...
- Tier metadata: `.dialectic-tier` — single word (`light`, `iterative`, `standard`, or `full`), lives on feature branches only, never on `main`
