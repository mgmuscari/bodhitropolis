...
#
# Exit codes:
#   0 = allow (not on feature branch; light/iterative/workflow tier; non-src file;
#       or a recognized team/workflow agent)
#   2 = block (standard/full tier, editing src/tests without a team)
#
# Tiers that permit direct edits: light (small changes), iterative (benchmark/
# debug loops), and workflow (codebase-scale /dialectic-sweep and
# /dialectic-migrate runs). Only standard/full require a team.
...
case "$TIER" in
  standard|full) ;;
  # light, iterative, and workflow tiers allow direct implementation.
  # workflow = codebase-scale dynamic-workflow audits/migrations
  # (/dialectic-sweep, /dialectic-migrate), whose subagents edit directly.
  light|iterative|workflow) exit 0 ;;
  # Unknown tier values are treated as non-blocking (fail open).
  *) exit 0 ;;
esac
