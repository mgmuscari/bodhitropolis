...
## Optional: drive execution to green with `/goal`

For a long PRP, you can set a completion condition so execution runs unattended but bounded —
without abandoning any of the rules above:

```text
/goal every task in the PRP is committed and the full validation gate suite from the PRP exits 0,
surfacing the command output each turn, or stop after N turns
```

The evaluator judges only what you surface in the transcript, so print the gate results each turn.
This automates the per-task loop; it does **not** change the TDD contract (RED first, never weaken
tests) or the atomic-commit rule.
