...
- If teammates haven't started within 90 seconds:
  ...
  3. **STOP** and report to user:
     "Team spawning failed. Teammates did not start within 90 seconds.
      This is likely Claude Code bug #32368 (model inheritance). (Bug #24316, agent definitions not loading, was resolved 2026-04-22.)
      Options:
...
## Optional: bound orchestration with `/goal`

As team lead you can set a completion condition so you keep monitoring/unblocking turn after turn
until the exchange genuinely converges, without manual re-prompting:

```text
/goal both teammate tasks show completed in TaskList AND the review artifact
docs/reviews/code/{slug}-review.md exists, or stop after N turns
```

Surface the `TaskList` state each turn so the evaluator can judge it. This does not change the
message-gating protocol or the teammates' work — it only keeps the lead engaged until done.
