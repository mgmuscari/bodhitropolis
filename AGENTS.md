...
This project is configured for use with **OpenCode** as the agentic development harness, running **local inference** via LM Studio with `qwen3.6-35b-a3b`.

### Configuration

- **Model:** `lmstudio/qwen3.6-35b-a3b` (configured in `opencode.json`)
- **Temperature:** 0.3 (balanced for code generation)
- **LM Studio:** Must be running with the model loaded (`http://localhost:1234/v1`)

### Primary Agents

| Agent | Switch | Purpose |
|-------|--------|---------|
| **Build** | `Tab` | Full development — write code, run commands |
| **Plan** | `Tab` | Analysis and planning — no file modifications |
...
|-------|------------|---------|
| `@proposer` | Full edit + bash | Implement features, produce artifacts |
| `@interlocutor` | No edit, limited bash | Stress-test plans for weaknesses |
| `@code-reviewer` | No edit, limited bash | Review code against spec and conventions |
| `@security-auditor` | No edit, full bash | Adversarial security testing |
| `@skeptical-client` | No edit, no bash | Challenge audit findings |

### Quick Start

```
# Start a new feature
@proposer Describe: add user authentication with OAuth2

# Review a plan
@interlocutor Review the PRP at docs/PRPs/auth-flow.md
...
See `INSTRUCTIONS.md` for complete documentation on the OpenCode harness, including workflow stages, permission configuration, troubleshooting, and best practices.
...
**Agent definitions load for team members.** Claude Code bug [#24316](https://github.com/anthropics/claude-code/issues/24316) (where `subagent_type` was ignored when `team_name` was set, spawning teammates as `general-purpose` with full tool access) is **resolved** — verified 2026-04-22 with a live probe: both the `tools:` frontmatter and the body prose load. Tool restrictions are enforced from `.claude/agents/*.md` for team and sequential spawns alike. The inline-stance workaround has been removed from the team commands.

**Caveat:** keep every tool a stance's body prose references in that stance's `tools:` grant — frontmatter and body must stay consistent (a partial load that restricted `tools:` while dropping body priming is what silently broke stances before the fix).

**Model setting:** All agent definitions use `model: opus` explicitly. Bug [#32368](https://github.com/anthropics/claude-code/issues/32368) (`model: inherit` / team model inheritance not resolving) was verified fixed by live probe on 2026-06-01 — team agents spawned with no explicit `model:` ran capably without going idle or degrading. Explicit `model: "opus"` is retained in agent defs and team spawn calls as cheap defense-in-depth.
