...
This directory contains the canonical, platform-agnostic definition of the Dialectic methodology. These templates are the source of truth — platform adapters (in `adapters/`) wire them into each platform's native format.

## Architecture

Dialectic uses a three-layer architecture:
...
| [Interlocutor](stances/interlocutor.md) | Patient, perceptive, structurally attuned | Plan review |
...
| [review-plan](workflows/review-plan.md) | PRP review | Interlocutor |
...
- **Dynamic workflows (scale tier)**: The `Workflow` tool — `/dialectic-sweep` (codebase-scale audit) and `/dialectic-migrate` (codebase-scale migration) — is Claude Code-only with no OpenCode/portable equivalent. This is a deliberate divergence: Claude Code is the reference implementation. On other backends, scope work down to a single branch and use the team or sequential path. See `docs/guides/dynamic-workflows.md`.
...
For the complete methodology specification, see [dialectic.md](../dialectic.md).
