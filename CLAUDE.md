...
2026-06-18: **ART/ASSETS — Bodhitropolis is the PRODUCT; Micropolis is reference/inspo ONLY.** The Micropolis code and the `micropolis-graphics/` assets exist for understanding the problem shape (tile vocabulary, sim behavior) — they are NEVER shipped. **No Micropolis pixel ever ends up in the game.** All generated artwork must be ORIGINAL art for Bodhitropolis's own `BuiltKind` vocabulary (`src/engine/fabric.ts`), produced via the ComfyUI pixel-art pipeline (remote server `https://comfyui.tailea7e08.ts.net`, configured via `COMFYUI_URL` in the comfyui-mcp plugin's `.mcp.json` — the ACTIVE copy is the cache one at `~/.claude/plugins/cache/comfyui-mcp/comfy/0.1.0/.mcp.json`, not the marketplace source; edits need an MCP reconnect). Architectural finding driving the pipeline: the live renderer is **100% procedural Canvas2D** (`src/ui/renderer.ts` → `buildAtlas`/`paintForKey`, 16×16 tiles, ~450 keys, zero image files; sprites are colored rects). So the split is — **keep procedural** for the *systemic/tiling* layer (terrain dither, road/rail connection masks, power lines; diffusion can't do seamless mask-aware 16×16), **generate** the *identity* layer (the ~35 building/landmark/tech-tree kinds) as original pixel art, downscaled to crisp tiles and loaded into the atlas as committed PNGs (determinism holds). Requirements: `docs/art/asset-generation.md`.
