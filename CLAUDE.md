...
2026-06-19: ⚠️ **The MCP Playwright browser is MADDY'S REAL Chrome, not a separate headless instance.** `browser_navigate` / reloading WIPES her in-progress playtest session (live state is recomputed per page-load; there is no save) — I nuked her active urban-renewal session this way. → While she is actively playing, do NOT navigate/reload the shared browser. Live-verification via `window.bodhitropolis` is fine to READ, but a fresh `?nointro=1` load regenerates the world. For dev verification, confirm she's not mid-session first (or rely on the unit tests + the HMR that already hot-applies source edits to her page). See [[live-verification-techniques]].
...
2026-06-20: DON'T delete-then-rebake sprite/tile PNGs while Maddy is actively playing — the live ambient/tileset loaders fetch once at load, so during the delete window a re-fetch (HMR re-run / her reload) 404s the files and the renderer falls back (peds → white MODE_COLOR boxes, cars → CAR_COLORS squares). It looks like a code regression but it's the missing files. → Re-bake only when she's not mid-session, or bake to a temp name and swap atomically; always rebuild the 4173 preview after a re-bake so a reload restores the sprites. (Distinct from a sprite baked AS a solid white box — that's a bad bake; the ambient bake now rejects it via `overallOpacity < 0.85`.)

2026-06-20: The white→alpha floodfill (`whiteToAlpha`) historically leaked through NARROW CHANNELS into interior white regions (ate the bus windows, clinic roof, light cars/mattress — a recurring vibe-killer). FIX (Maddy: "flood fill needs to be unable to pass thru narrow channels"): build the removal mask as white-mask → ERODE by `neck` (sever channels ≤~2·neck px) → flood from an added white BORDER (true exterior only) → regrow into the eroded ring. Two bake guards now catch the class: `isIntact` (center not floodfilled-away) + `overallOpacity < 0.85` (not a solid box). `TS_NECK` tunes the neck radius. Tents/peds/long vehicles (buses) are the high-risk subjects (interior light pockets); always re-validate them after a bake.
